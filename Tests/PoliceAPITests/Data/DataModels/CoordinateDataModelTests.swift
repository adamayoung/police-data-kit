...
final class CoordinateDataModelTests: XCTestCase {
    ...
    func testLatitudeWhenNotANumberReturnsZero() throws {
        let result = try JSONDecoder.policeDataAPI.decode(CoordinateDataModel.self, fromResource: "coordinate-invalid").latitude
    ...
    func testLongitudeReturnsValue() throws {
        ...
        let result = try JSONDecoder.policeDataAPI.decode(CoordinateDataModel.self, fromResource: "coordinate").longitude

        XCTAssertEqual(result, expectedResult)
    }

    func testLongitudeWhenNotANumberReturnsZero() throws {
        let result = try JSONDecoder.policeDataAPI.decode(CoordinateDataModel.self, fromResource: "coordinate-invalid").longitude
