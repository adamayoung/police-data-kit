...
public final actor InMemoryCache: NSObject, Cache {

    private let cache: NSCache<NSString, CacheItem>
    private let defaultExpiresIn: TimeInterval
    private let logger: Logger

    public init(name: String, defaultExpiresIn: TimeInterval = 60 * 60 * 12, countLimit: Int = 0) {
        self.cache = NSCache()
    ...
    public func object<ObjectType: Any>(for key: some CustomStringConvertible,
                                        type: ObjectType.Type) async -> ObjectType? {
    ...
    public func set(_ object: Any?, for key: some CustomStringConvertible, expiresIn: TimeInterval? = nil) async {
        let cacheKey = keyValue(for: key)
